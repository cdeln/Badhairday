...
#ifndef INTEGER_TEXTURE
gtl::shader::HairTessVoxelizerOptimized hairVoxelizer;
gtl::shader::HairTessRendererOptimized hairRenderShader;
gtl::shader::VoxelClear voxelClear;
gtl::shader::VoxelCumsum voxelCumsum;
#else
gtl::shader::IntegerHairTessVoxelizerOptimized hairVoxelizer;
gtl::shader::IntegerHairTessRendererOptimized hairRenderShader;
gtl::shader::IntegerVoxelClear voxelClear;
gtl::shader::IntegerVoxelCumsum voxelCumsum;
#endif
...
float shadeFactor = 20.0;
float lightYaw = 0;
float lightPitch = 0;
int innerTessLevel = 4;
...
void updateTime() {
    ...
    ballPos = glm::vec3(2*sin(0.001*ballTime), 0, 2*cos(0.001*ballTime));
...
void drawDepth() {
    ...
    depthRenderer.use();
    depthRenderer.transform = light.get() * modelMatrix;
...
void drawModel() {
    ...
    if( ! displayMeshDensity ) {
        ...
        phongShader.tessLevel = innerTessLevel;
        phongShader.outerTessLevel =  outerTessLevel;
    ...
    if( displayMeshDensity ) {
        ...
        meshDensityShader.tessLevel = innerTessLevel;
...
void drawHair() {
    ...
    hairRenderShader.MVP = projectionMatrix * modelViewMatrix;
    hairRenderShader.modelViewMatrix = modelViewMatrix;
    hairRenderShader.normalMatrix = normalMatrix;
    glm::mat4 lightVP = light.get(); //lightProj * lightView;
    lightVP = lightVP * modelMatrix;
    hairRenderShader.lightMatrix = lightVP;
    glm::vec3 lightPos = normalMatrix * light.getPos();
    hairRenderShader.light = lightPos;
    //hairRenderShader.hairLength = hairLength;
    hairRenderShader.shadeFactor = shadeFactor;
    hairRenderShader.forceCenter = ballPos;
    hairRenderShader.tex = hairTex;
    hairRenderShader.depthSampler = 2; // depth texture
    hairRenderShader.innerTessLevel = innerTessLevel;
    hairRenderShader.outerTessLevel =  outerTessLevel;
...
void voxelize() {
    ...
    */
    ...
    hairVoxelizer.innerTessLevel = innerTessLevel;
    hairVoxelizer.outerTessLevel = outerTessLevel;
