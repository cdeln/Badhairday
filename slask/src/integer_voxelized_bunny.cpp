...
gtl::shader::IntegerHairTexLightRender hairRenderShader;
...
gtl::shader::IntegerHairVoxelizer hairVoxelizer;
gtl::shader::IntegerVoxelizer voxelizer;
...
float shadeFactor = 0.2;
float lightYaw = 0;
float lightPitch = 0;
int tessLevel = 3;
bool forwardInterpolate = true;
...
void keyboardCallback(unsigned char key, int x, int y) {
    ...
    if( key == '3' ) {
        ...
        layerFactor = std::max(1, layerFactor - 1);
    }
    else if (key == 'y' ) {
        layerFactor += 1;
    }
...
void init(void) {
    ...
    voxelFBO.allocate(fboWidth, fboHeight, fboDepth);
...
void drawHair() {
    ...
    hairRenderShader.gatherInterpolate = backwardInterpolate;
...
void voxelize() {
    ...
    */
    ...
    hairVoxelizer.forwardInterpolate = forwardInterpolate;
